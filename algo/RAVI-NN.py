...
class QNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, reward_dim, hidden_dim=128):
        ...
        self.fc1 = nn.Linear(state_dim + action_dim + reward_dim + 1, hidden_dim)
...
class RAVI_NN:
    def __init__(self, env, discre_alpha, growth_rate, gamma, reward_dim, time_horizon, welfare_func_name, nsw_lambda, save_path, seed=1122, p=None, threshold=5, wdb=False, scaling_factor=1) -> None:
        ...
        # Buffer to store (state, action, Racc_code, t, reward, next_state, next_Racc_code)
        self.replay_buffer = []
    ...
    def initialize(self):
        ...
        for _ in tqdm(range(num_samples), desc="Initializing Q-network..."):
            ...
            Racc = np.random.rand(self.reward_dim) * self.time_horizon  # Randomly sample accumulated reward
            ...
            input_tensor = torch.cat((state_tensor, action_tensor, Racc_tensor, t), dim=-1)
    ...
    def train(self):
        ...
        for t in tqdm(range(1, self.time_horizon + 1), desc="Training..."):
            for _ in range(num_samples):
                state = np.random.randint(self.env.observation_space.n)
                Racc = np.random.rand(self.reward_dim) * (self.time_horizon - t)  # Randomly sample accumulated reward
                ...
                next_Racc_code = next_Racc  # Directly use next_Racc instead of encoding

                for a in range(self.num_actions):
                    self.replay_buffer.append((state, a, Racc, t, reward_arr[a], next_state_arr[a], next_Racc_code, transition_prob[a]))
    ...
    def evaluate(self, final=False):
        ...
        for t in range(self.time_horizon, 0, -1):
            ...
            input_tensor = torch.cat((state_tensor, Racc_tensor, t_tensor), dim=-1)
            action = torch.argmax(self.q_network(input_tensor)).item()
